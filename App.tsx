...
const MarkdownDisplay: React.FC<{ content: string }> = ({ content }) => {
  ...
  return <div className="markdown-body leading-relaxed text-[15px] text-gray-800" dangerouslySetInnerHTML={{ __html: processMarkdown(content) }} />;
};
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(saveTasks, 500); 
    ...
    const interval = setInterval(() => {
      ...
      tasks.forEach(async (task) => {
        if (task.status === TaskStatus.PENDING && new Date(task.nextRun) <= now) {
          // Pre-emptively update to prevent double trigger
          setTasks(current => current.map(t => t.id === task.id ? { ...t, status: TaskStatus.COMPLETED } : t));
          ...
          if (Notification.permission === "granted") {
            new Notification(task.description, { 
              body: "Pulse automation executed successfully.", 
          ...
          setMessages(prev => [...prev, {
            ...
            role: 'model',
            content: `### 🤖 Task Executed: ${task.description}\n\n${result}`,
    ...
    try {
      ...
      if (parsed && parsed.nextRun) {
        ...
        setMessages(prev => [...prev, {
          id: crypto.randomUUID(),
          role: 'model',
          content: "I couldn't quite set up that schedule. Try something like: 'Get financial news every day at 8 AM'.",
          timestamp: new Date().toISOString()
        }]);
      }
    } catch (e) {
      setMessages(prev => [...prev, {
        id: crypto.randomUUID(),
        role: 'model',
        content: "Configuration Error: Make sure the server API_KEY is set correctly in the VPS .env file.",
        timestamp: new Date().toISOString()
      }]);
    } finally {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F2F2F7] safe-area-inset-top select-none">
      {/* Header */}
      <header className="px-6 py-4 bg-white/90 backdrop-blur-xl border-b border-gray-200 sticky top-0 z-50 flex justify-between items-center shadow-sm">
        ...
        <div className="flex items-center space-x-2 text-[10px] font-bold text-gray-400">
          ...
          <span className="hidden sm:inline">VPS ENGINE ONLINE</span>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-hidden flex flex-col md:flex-row">
        ...
        {/* Task List */}
        <aside className="w-full md:w-[380px] p-6 overflow-y-auto space-y-6 bg-gray-50/50">
          ...
          <div className="space-y-4">
            {tasks.filter(t => t.status === TaskStatus.PENDING).length === 0 ? (
              <div className="text-center py-20 bg-white/50 rounded-[30px] border-2 border-dashed border-gray-200">
                <p className="text-[10px] font-black text-gray-300">QUEUE EMPTY</p>
              </div>
            ) : (
              tasks.filter(t => t.status === TaskStatus.PENDING).map(task => (
                <div key={task.id} className="bg-white p-5 rounded-[30px] shadow-sm border border-gray-100 group hover:shadow-xl transition-all duration-300">
                  ...
                  <p className="text-[14px] font-bold text-gray-900 leading-tight mb-3">{task.description}</p>
                  <div className="flex items-center text-[10px] text-gray-400 font-bold">
                    ...
                    Next Run: {new Date(task.nextRun).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                  </div>
                </div>
              ))
            )}
          ...
          <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-md p-0 sm:p-4 animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-2xl rounded-t-[40px] sm:rounded-[40px] overflow-hidden shadow-2xl animate-in slide-in-from-bottom-10 duration-500">
              <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                <h3 className="text-xl font-black text-gray-900">Task Control</h3>
              ...
              <div className="p-8 space-y-8 max-h-[75vh] overflow-y-auto">
                ...
                <section className="bg-gray-50 p-8 rounded-[36px] space-y-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-black text-gray-900">Persistent Backend</p>
